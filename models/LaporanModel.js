...
export const getLaporan = async (selectedLocation) => {
  try {
    let sql = `
      SELECT 
          cp.no_do,
    ...
    // Jika lokasi dipilih, tambahkan kondisi WHERE untuk memfilter
    if (selectedLocation) {
      sql += ` WHERE l.lokasi = ?`;
    }

    const [rows] = await db.execute(sql, selectedLocation ? [selectedLocation] : []);
